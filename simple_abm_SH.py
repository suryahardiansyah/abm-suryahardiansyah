...
params = {'world_size':(10,10),
          ...
          'same_pref' :0.2,
          'max_iter'  :25,
          'out_path'  :r'D:\UCHICAGO\DATA ANALYSIS PYTHON I\W9_Lab\abm_results_SH.png' }

class Agent():
    def __init__(self, color, world, same_pref):
        """An agent, either blue or red, is inhabiting a world 
        with another agent(s). He knows his location and surrounding patches."""
        self.color = color
    ...
    def move(self):
        """Agent needs to move to a new position if he is not happy, 
        but sometimes he can't move."""
    ...
    def am_i_happy(self, loc=False, neighbor_check=False):
        """This returns a boolean for whether or not an agent is happy 
        at a location. 
        ...
        neighbor_colors   = [agent.color for agent in neighbor_agents if agent is not None]
        num_like_me      = sum([color == self.color for color in neighbor_colors])

        #for reporting purposes, allow checking of the current number of similar neighbors
        if neighbor_check:
            return [color == self.color for color in neighbor_colors]

        #if an agent is in a patch with no neighbors at all, treat it as unhappy
        if len(neighbor_colors) == 0:
            return False

        perc_like_me = num_like_me / len(neighbor_colors)
...
class World():
    def __init__(self, params):
        assert(params['world_size'][0] * params['world_size'][1] > params['num_agents']), 'Grid size is too small for number of agents.'
        self.params = params
        self.reports = {}
    ...
    def build_grid(self, world_size):
        """create the world grid by grid that the agents can move around on"""
    ...
    def build_agents(self, num_agents, same_pref):
        ...
        agents = [Agent(self, _color_picker(i), same_pref) for i in range(num_agents)]
